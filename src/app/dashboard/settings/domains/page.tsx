...
import Card from "@/src/components/settings/domains/card";
...
import { useDomains } from "@/src/services/domains/service";
import DomainDialog from "@/src/components/settings/domains/dialog";

export default function Page() {
  const { data, isLoading, refetch } = useDomains();
   ...
   if(products){
    if (search) {
        const filteredProducts = products.filter((product: any) => {
          ...
          const platform = nfd(product.platform.toLowerCase());
  ...
  return (
    <>
      ...
      >
        <Header
          title="Gestión de dominios reservados"
          message="Aquí puedes gestionar los dominios reservados para la validación de alertas de inversión extranjera directa."
        />
        <div className="flex flex-col w-full p-4 space-y-2 sm:p-8 sm:space-y-0 sm:flex-row">
          <div className="flex items-center justify-start w-full text-xl font-semibold text-center text-black sm:text-left sm:w-4/12">
            Cantidad de dominios reservados: {total}
          </div>
          ...
          {currentPageData?.length === 0 ? (
            ...
            <>
              <div className="grid items-center justify-between w-full h-24 grid-cols-3 p-5 font-bold text-center bg-white rounded-lg ring-2 ring-gray-100">
                ...
                <div className="text-center">Plataforma</div>
                <div>Acción</div>
              </div>
