...
import SCard from "@/src/components/settings/alertacomercial/card";
import AlertaComercialDialog from "@/src/components/settings/alertacomercial/dialog";
import { Select } from "@mantine/core";
import Header from "@/src/components/settings/header";
import useAlertaComerciales, {
  useAlertaComercialesCategory,
  useAlertaComercialesPage,
...
export default function Page() {
  ...
  const [filterAlertaComerciales, setFilterAlertaComerciales] = useState([]);
  ...
  const { data: d, refetch } = useAlertaComerciales();
  useEffect(() => {
    setData(d);
    setTotal(d?.length);
  }, [d]);
  ...
  const filterData = () => {
    ...
    if (!status) {
      ...
      filteredBySearch = filteredByCategory.filter(
        (alertacomercial) =>
          alertacomercial.status.toLowerCase() === status.toLowerCase() &&
          (alertacomercial.title.toLowerCase().includes(search.toLowerCase()) ||
            alertacomercial.products.some(
              (product) =>
                product.name.toLowerCase().includes(search.toLowerCase()) ||
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      names.unshift("Todos");
      console.log(names);
      //setFilterAlertaComerciales(names);
    }
  }, [categories, isLoading]);

  const statusAlertaComerciales = [
  ...
  const handleFilter = (selectedCategory: string | null) => {
    ...
    if (categoryToFilter.toLowerCase() == "todos") {
      const AlertaComercialByCategory = data.filter(
        (alertacomercial) =>
          alertacomercial.status.toLowerCase() === statusToFilter.toLowerCase()
      );
      setCategory(categoryToFilter);
      setFilteredData(AlertaComercialByCategory);
      setTotal(AlertaComercialByCategory?.length);
    } else {
      const AlertaComercialByCategory = data.filter(
        (alertacomercial) =>
          alertacomercial.category.name.toLowerCase() ===
  ...
  const handleStatus = (selectedStatus: string | null) => {
    ...
    const AlertaComercialByStatus =
      categoryToFilter !== "Todos"
        ? data.filter(
            (alertacomercial) =>
              alertacomercial.status.toLowerCase() ===
                statusToFilter.toLowerCase() &&
              alertacomercial.category.name.toLowerCase() ===
                categoryToFilter.toLowerCase()
          )
        : data.filter(
    ...
    setFilteredData(AlertaComercialByStatus);
    setTotal(AlertaComercialByStatus?.length);
  };
